...
impl From<UploadImageInput> for CreateImageData {
    fn from(data: UploadImageInput) -> Self {
        Self {
            id: None,
...
impl From<CreateImageInput> for CreateImageData {
    fn from(data: CreateImageInput) -> Self {
        Self {
            id: data.id,
            user_id: data.user_id,
...
impl ImageMutation {
        ...
        let insert_data = match role {
            Role::User => {
                ...
                let insert_data = if let Some(user_id) = data.user_id {
                    user_id_guard(ctx, user_id)?;
                    CreateImageData::from(data)
                } else {
                    CreateImageData::from(data.set_user_id(user_id))
                };

                insert_data
            }
            Role::Admin => CreateImageData::from(data),
            Role::Guest => return Err(async_graphql::Error::new("User not logged in")),
        };
    ...
    pub async fn delete_image(&self, ctx: &Context<'_>, id: Uuid) -> async_graphql::Result<Image> {
        ...
        let upload_dir: std::path::PathBuf = env::var("UPLOAD_FOLDER")
            ...
            .into();
        let upload_file = upload_dir.join(
            image
                .id
                .to_hyphenated_ref()
